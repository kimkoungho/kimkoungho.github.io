...
## 쿠버네티스의 역할 
도커는 컨테이너를 관리하는 데몬인 dockerd 와 명령행 도구로 구성됨 
쿠버네티스는 스웜 보다 더 많은 기능을 갖춘 컨테이너 **오케스트레이션 시스템** 으로 컨테이너 런타임을 다룰 수 있음 


> Minikube
> 원도우/mac OS 용 쿠버네티스 연동 기능이 추가되기 전까지는 로컬 환경에 쿠버네티스 환경 구축을 위해 사용하던 도구
> 로컬 환경에서 쿠버네티스를 사용가능하게 되면서 Minikube 는 점점 덜 사용하고 있음 
> 쿠버네티스는 도커의 dockered 를 사용하여 환경을 구축하지만 
> Minikube 는 새로운 dockered 를 띄워서 쿠버네티스 환경을 구축하는 방식으로 2개의 dockered 가 뜨게 된다 
> 새로운 dockered 를 사용하기 때문에 도커-쿠버네티스 연동버전 보다 좀 더 까다로운 면이 있음 
